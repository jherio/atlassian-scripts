...
if (object) {
    ...
    def createUserRequest = UserService.CreateUserRequest.withUserDetails(curUser, insUserMail.getValue().toString(), null, insUserMail.getValue().toString(), insUserName.getValue().toString()).withNoApplicationAccess()

    def createUserResult = userService.validateCreateUser(createUserRequest)
    if( createUserResult.isValid() ) {
        def newUser = userService.createUser(createUserResult)
        ...
        def insattrJiraUsersNew = objectAttributeBeanFactory.createObjectAttributeBeanForObject(object, insattrJiraUsersType, newUser.getUsername())
        ...
        log.warn( createUserResult.getErrorCollection() )
    }
